...
const FontLoader = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .pub-row { transition: background 0.18s; border-radius: 4px; }
    ...
    .pub-img { border-radius: 4px; object-fit: cover; box-shadow: 0 4px 16px rgba(0,0,0,0.10); border: 1px solid var(--rule); }
  `}</style>
);
...
/* ─── PubRow with optional image ────────────────────────────── */
const PubRow = ({ pub, tag, tagClass }) => (
  <div
    className="pub-row"
    style={ borderTop: '1px solid var(--rule)', padding: '28px 4px', cursor: pub.doi ? 'pointer' : 'default' }
      ...
      {pub.image && (
        <div style={ flexShrink: 0 }>
          <img
            ...
            className="pub-img"
            style={{
              width: pub.imageWide ? '180px' : '90px',
              height: pub.imageWide ? '100px' : '120px',
              objectFit: 'cover',
              objectPosition: pub.imageWide ? 'top left' : 'center',
            }}
          />
        </div>
      )}
...
export default function Portfolio() {
  ...
  const journals = [
    ...
    },
    {
      ...
      year: 'In Press', doi: null, image: null,
  ...
  const bookChapters = [
    ...
    },
    {
      ...
      image: IMG_ASME, imageWide: false,
  ...
  const projects = [
    ...
    },
    {
      ...
      desc: 'A full-stack Scientific Machine Learning framework for algorithmic trading research. Trains RL agents (PPO, SAC, TD3, A2C, DDPG) to learn adaptive trading strategies. Includes market regime detection, portfolio dynamics modeling, and rich visualization covering equity curves, drawdown analysis, and return distributions. Bridges CFA knowledge with computational finance research.',
    },
    {
      ...
      desc: 'A Retrieval-Augmented Generation assistant for financial news analysis. Ingests live financial news, embeds content into a vector store, and enables analysts to query, summarize, and reason over current market events using an LLM backend — surfacing non-obvious cross-asset signals from large news corpora.',
    },
    {
      ...
      desc: 'A contribution to and fork of the RustQuant quantitative finance library — exploring high-performance financial computation in Rust. Covers option pricing models (Black-Scholes, Monte Carlo), yield curve construction, and risk metric computation.',
    },
    {
      ...
      desc: 'An AI-powered mental wellness companion for young users experiencing emotional distress. Uses NLP-based mood detection and empathetic response generation to provide coping strategies, journaling prompts, and mood trend tracking. Informed by user research principles from the NextLeap PM Fellowship.',
    },
    {
    ...
    },
    {
      ...
      desc: 'A product-thinking-first dating application using AI recommendation systems to improve match quality beyond superficial swiping. Uses semantic compatibility scoring, conversation quality analysis, and behavioral pattern learning to surface better long-term compatibility signals.',
    },
    {
      ...
      desc: 'A fully voice-driven travel planning agent that interprets natural language requests to research destinations, compare flights and stays, and build complete itineraries through conversational interaction. A direct prototype for voice AI capabilities being developed in Arogyavani.',
    },
  ];
  ...
  const pmProjects = [
    {
      ...
      desc: 'Designed a monetisation framework for Google Maps targeting local SMB owners in Tier-2 and Tier-3 Indian cities. Identified the gap between high map traffic and low conversion for local merchants. Proposed a freemium listing upgrade model with performance-based ad bidding, appointment booking integration, and a "Verified Local" trust badge. Modeled revenue impact across user segments and outlined a phased GTM strategy starting with high-intent verticals.',
    ...
    },
    {
      ...
      desc: 'Conducted a heuristic evaluation of PhonePe's merchant-facing dashboard, identifying pain points around transaction reconciliation, settlement timing transparency, and dispute resolution workflows. Proposed a redesigned experience with real-time P&L visibility, one-click settlement requests, and an AI-generated weekly business digest. Grounded in merchant interviews and competitive benchmarking against Razorpay and Stripe.',
  ...
  const ventures = [
    ...
    },
    {
      ...
      desc: 'Arogyavani addresses a critical gap in India's primary healthcare delivery: the breakdown in communication between doctors and patients after a consultation ends. The platform provides clinics with an AI voice assistant handling appointment scheduling, post-visit follow-ups, prescription reminders, and basic symptom triage — all in regional languages. Built on a lightweight CRM for small clinics, reducing patient drop-off and freeing clinic staff from repetitive coordination. Pilot clinics onboarded in Tamil Nadu.',
  ...
  const achievements = [
    ...
    { exam: 'State Vigyan Paheli',    score: 'State Rank 7',      year: 'Mar 2016' },
    { exam: 'Bal Vigyan Competition', score: 'State Rank 2',      year: '2014' },
  ];

  const orgs = [
    {
      name: 'SciML OpenLab', url: 'https://github.com/SciML-OpenLab',
    ...
    {
      name: 'Curious-Ind', url: 'https://github.com/Curious-Ind',
      desc: 'A collaborative open-source initiative exploring AI, product, and engineering experiments. Projects range from LLM tooling and agent frameworks to data-driven product analytics and consumer AI applications.',
            ...
            <div style={ display: 'grid', gridTemplateColumns: '190px 1fr', gap: '44px', marginBottom: '64px' }>
              ...
              <div>
                ...
                <div style={ background: 'rgba(255,248,237,0.9)', border: '1px solid var(--rule)', borderLeft: '3px solid var(--saffron)', borderRadius: '4px', padding: '20px 24px', marginBottom: '20px' }>
                  ...
                  <div style={ display: 'flex', flexDirection: 'column', gap: '14px' }>
                    {[
                      ...
                      { area: 'Business Development', detail: 'Support the commercial pipeline — preparing client proposals, techno-commercial reports, and presentation decks for institutional and government buyers. Engage directly with stakeholders during pre-sales and technical discussions.' },
                    ].map((r, i) => (
                      <div key={i}>
                ...
                <p className="sans" style={ fontSize: '0.88rem', lineHeight: 1.84, color: 'var(--ink-light)', marginBottom: '18px' }>
                  Joined Tvasta immediately after graduation as part of a structured engineering rotation. Worked across design, process engineering, and operations to understand the full lifecycle of robotic construction. Contributed to workflow optimization projects that reduced material waste in 3D concrete printing and supported business development including client proposals and competitive benchmarking.
                </p>
                <div style={ display: 'flex', flexWrap: 'wrap', gap: '7px' }>
              ...
              <div>
                {[
                  { title: 'Research Intern — Microfluidics & Scientific ML', period: '2023 – 2024', type: 'Research',
                    desc: 'Conducted research under Prof. Karthick Subramani in acoustofluidics, droplet dynamics, and Scientific Machine Learning. Focused on how acoustic radiation forces govern particle and droplet behavior in microfluidic channels. Developed Physics-Informed Neural Networks (PINNs) using optimal transport methods for high-fidelity turbulence modeling. Produced two journal publications, four international conference presentations (India, UK, Italy), and multiple book chapters.',
          ...
          <Reveal delay={0.12}>
            <div style={ display: 'grid', gridTemplateColumns: '190px 1fr', gap: '44px' }>
              ...
              <div>
                ...
                <p className="sans" style={ fontSize: '0.88rem', lineHeight: 1.84, color: 'var(--ink-light)', marginBottom: '18px' }>
                  Completed NextLeap's highly selective Product Management Fellowship, ranked among the top cohort participants. Covered end-to-end product thinking — discovery, user research, metrics, prioritization, and GTM strategy. Worked on live PM case studies and product teardowns across B2B SaaS and consumer tech.
                </p>
                <div style={ display: 'flex', flexWrap: 'wrap', gap: '7px' }>
        ...
        <W>
          ...
          <Reveal delay={0.1}>
            ...
            <div style={ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }>
              {orgs.map((o, i) => (
                <div key={i} onClick={() => window.open(o.url, '_blank')} className="proj-card"
        ...
        <W>
          ...
          <Reveal delay={0.2}>
            <a href="https://nextleap.app/portfolio/anjan-mahapatra" target="_blank" rel="noopener noreferrer"
              style={ display: 'inline-flex', alignItems: 'center', gap: '8px', marginTop: '28px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.82rem', fontWeight: 500, color: 'var(--saffron)', textDecoration: 'none' }>
